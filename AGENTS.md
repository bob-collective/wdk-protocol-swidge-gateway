...
- **The gateway `/v3` API is geofenced (EU-only).** CI validation and the swap harness must run on the **EU self-hosted runner** (`bob-ubuntu-latest`); github-hosted runners get a Cloudflare **403**.
