...
test('offramp: maps amounts and fees correctly', () => {
  ...
  expect(q.fees).toEqual(expect.arrayContaining([
    expect.objectContaining({ type: 'network', amount: 500n })
  ]))
  expect(q.fees).toEqual(expect.arrayContaining([
    expect.objectContaining({ type: 'protocol', amount: 150n })
  ]))
  expect(q.fees).toEqual(expect.arrayContaining([
    expect.objectContaining({ type: 'affiliate', amount: 100n })
  ]))
})
...
test('onramp: maps amounts and fees, no inclusionFee so network = solverFee only', () => {
  ...
  expect(q.fees).toEqual(expect.arrayContaining([
    expect.objectContaining({ type: 'protocol', amount: 200n })
  ]))
  expect(q.fees).toEqual(expect.arrayContaining([
    expect.objectContaining({ type: 'affiliate', amount: 75n })
  ]))
})
